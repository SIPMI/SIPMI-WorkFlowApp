	...
	private static void registWorkList(Integer workflowId, List<String> taskList, List<String> varList){
		...
		for(String task : taskList){
			...
			if(task.startsWith("InputData")){
				// ワークに対応するパラメータの登録
	    		TbWorkParamDao.registWorkParam(workId, parentId, "InputData", ImageUtil.createImageStringFromPath(varList.get(0)));
	    	}//1をsortnoに変えれば？2つめしか表示されなくなった
			else if("math_number".equals(task)){
				// ワークに対応するパラメータの登録
				TbWorkParamDao.registWorkParam(workId, parentId, "InputVal", varList.get(1));
	    	}
